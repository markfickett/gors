...
When invoked, gors requests that each of the objects representing feeds check for updates. Each feed object starts a new thread which fetches the RSS feed's source, parses the returned XML, and stores the latest available item's guid attribute; the feed object then notifies gors (via callback). For any updated feeds, gors opens the main URL in the OS-designated web browser.
